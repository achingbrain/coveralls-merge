...
function convertLcovSectionToCoverallsSourceFile(lcovSection, projectRoot) {
    ...
    lcovSectionLines.forEach(function (line) {
        if (line.startsWith('SF:')) {
            ...
            coverallsSourceFile.name = (0, _helpers.getRelativeFilePath)(absoluteFilePath, projectRoot);
...
exports.default = function (reportFile, config) {
    return new Promise(function (resolve) {
        var lcovReportFilePath = _path2.default.resolve(config.projectRoot, reportFile),
            lcovContents = (0, _helpers.getSourceFromFile)(lcovReportFilePath),
            lcovSections = lcovContents.split('end_of_record\n'),
            result = lcovSections.filter(emptySections).map(function (section) {
            return convertLcovSectionToCoverallsSourceFile(section, config.projectRoot);
        });
